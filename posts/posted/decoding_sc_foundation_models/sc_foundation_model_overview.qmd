...
3. Download the [`sc_foundation_model_overview.qmd`](https://github.com/shackett/shackett/blob/master/posts/posted/sc_foundation_model_overview.qmd) notebook (or copy and paste the relevant code blocks).

4. Configure `MODEL_OUTPUTS_DIR` and other paths in the `env_setup` code block to point to your local directories.
...
# Napistu
from napistu.utils import save_pickle
...
from sc_foundation_model_utils import get_model_label_maps, get_model_names
from sc_foundation_model_overview_helpers import (
    aggregate_model_categories,
    compute_within_model_layer_comparisons,
    get_cache_path,
    get_model_categories,
    get_model_comparison_metadata,
    model_facet_mosaic_layout,
    plot_rank_agreement_by_layer,
    residual_corr_to_matrix,
)
...
display(model_metadata_summary)
```
