...
Using this information, we can assess how likely an observation is to have come from the underlying noise estimate using a Wald test. Because we are working with fold changes, the variance of fold-changes will be twice the variance of individual observations (the variance of a difference of normals equals the sum of the variances of each normal). With this noise estimate, observed fold change can be compared to the standard deviation estimate to yield a z-score and corresponding p-values. (One limitation of this approach is that it doesn't deal with the bias induced by time-zero normalization as discussed in **post**; If anyone knows of a better hypothesis test, please ping me :).
...
# Budgeting power with the functional FDR.


But, as we suspect that there will be stronger signals at later timepoints, it is helpful to stratify the p-value histograms by timepoint.
...
Visualizing p-value histograms separately by time, we can see that the p-values distributions are closer to $\text{Unif}(0,1)$ at early timepoints and there is a larger fraction of small p-values at late timepoints.
...
pi_zero_by_t <- estimate_fpi0(p = observation_signals$p_norm,
                              z0 = observation_signals$time, method = "glm")$table %>%
...
shrunken_timecourse_estimates <- lapply(timepoint_lists, function(a_time_set) {
  ...
  dplyr::bind_rows() %>%
  dplyr::mutate(shrunken_fold_change = fold_change * (1-lfdr))
```

```{r tc_shrinkage_examples, fig.height = 6, fig.width = 6}
shrunken_timecourse_estimates %>%
  inner_join(example_tcs, by = "tc_id") %>%
  dplyr::bind_rows(tibble::tibble(time = 0,
  ...
  geom_point(aes(y = shrunken_fold_change)) +
