...
*Additionally, while I am a current Calico employee, I want to emphasize that this and any other posts on my personal blog are my personal employees and do not reflect the opinions of my employer.*

# Background

## Life Tables

Demographic models of mortality and life expectancy are built from "life tables." Life tables contain both the number of individual of a given age that are living the selected year, and the number of deaths that occurred in these ages during the selected year. This is sufficient to calculate the probability of an individual of a given age dying that year (i.e., the hazard).
...
![Example Life Table](figures/gompertz/lifetable.png)
...
# read html
us_life_expectancy_html <- rvest::read_html("https://u.demog.berkeley.edu/~andrew/1918/figure2.html")
...
Finally, we can visualize the changes in life expectancy across the $20^{th}$ century. From a low point in 1918, life expectancy rose `r lifespan_extension_20th`-fold.

```{r life_expectancy, fig.height = 6, fig.width = 5.5}
...
```{r gompertz_basics, fig.height = 4, fig.width = 12}
...
life_expectancy_by_alpha %>%
  ...
  scale_y_continuous("Life expectancy") +
  labs(title = "20<sup>th</sup> century lifespan extension was<br>through a **26x** drop in baseline hazard") +
  theme_bw_mod +
  theme(
...
The nearly doubling of life expectancy across the $20^{th}$ century was chiefly driven by the massive medical advancements of antibiotics and vaccines, as well improvements in nutrition and hygiene. The deceleration of lifespan extension over the last thirty years reflects the challenge of removing risks one-by-one like in a game of wack-a-mole. Going forward, little is projected to change as judged by the social security administration, where lifespan extension in 2,100 is only projected to reach $\sim 85$ years

![SSA projected survival](figures/gompertz/ssa_projected_survival.png)
...
```{r beta_var, fig.height = 5, fig.width = 4.3}
...
![Lifespans based on unnatural deaths](figures/gompertz/polstats.png)
...
The maximum lifespan under this model would be equivalent to the maximum of $N$ geometric draws. This distribution is described in [StackExchange](https://math.stackexchange.com/questions/26167/expectation-of-the-maximum-of-i-i-d-geometric-random-variables) and its quite involved so I'll cop out and just obtain the maximum value of a set of random draws.

```{r}
current_earth_pop_w_geometric_lifespan <- rgeom(n = 7.9e9, ALPHA_CURRENT)
max(current_earth_pop_w_geometric_lifespan)
```
