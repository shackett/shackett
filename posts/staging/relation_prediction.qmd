...
uv pip install wandb==0.22.3 # temporary fix for wandb <0.23
uv pip install "napistu-torch[pyg,lightning,analysis]==0.3.0"
...
4. Set an appropriate working directory and download a version of the `NapistuGraph` which doesn't include reaction vertices:
...
WORKING_DIR = Path(os.path.expanduser("~/Desktop/EXPERIMENTS"))
PATH_TO_NAPISTU_DATA = WORKING_DIR / ".napistu_data"
PATH_TO_NAPISTU_STORE = WORKING_DIR / ".store"

napistu_data_store = gcs_model_to_store(
    napistu_data_dir = PATH_TO_NAPISTU_DATA,
    store_dir = PATH_TO_NAPISTU_STORE,
    asset_name = "human_consensus_no_rxns",
    asset_version = "20251218",
)
...
5. Choose your path:
   - **Using pre-trained models** (recommended): Download [pre-trained models and configs](https://github.com/shackett/shackett/blob/main/assets/data/relation_prediction_experiments.zip) (~50MB) and extract to your experiments directory.
...
# globals
EXPERIMENTS_DIR = Path(os.path.expanduser("~/Desktop/relation_prediction_experiments/"))
...
EXPERIMENTS = {
    ...
    "relation_attention" : "relation_prediction_relationattention_128e_v1",
    ...
    "rotate" : "relation_prediction_rotate_128e_transfer_v1",
    "transe" : "relation_prediction_transe_128e_transfer_v1",
}

RELATION_AWARE_FOCUSED_HEADS = ["distmult", "transe", "relation_attention", "relation_gated_mlp", "relation_attention_mlp"]
...
checkpoint_path = EXPERIMENTS_DIR / EXPERIMENTS["relation_gated_mlp"] / "checkpoints" / "last.ckpt"
...
# Pick a small set of test edges (e.g., 5 edges from your test set)
n_test_edges = 5
test_edge_indices = torch.where(napistu_data.test_mask)[0][:n_test_edges]
test_edges = napistu_data.edge_index[:, test_edge_indices]
test_edges_true_relation_types = napistu_data.relation_type[test_edge_indices]

print(f"\n=== Testing {n_test_edges} edges across all relation types ===")
print(f"Test edges shape: {test_edges.shape}")
print(f"True relation types: {[relation_types[i] for i in test_edges_true_relation_types]}")

# Score these edges under each relation type for one model
model = models["relation_gated_mlp"]
task = model.task

# Create a results dataframe
results = []

for relation_idx, relation_name in enumerate(relation_types):
    # Create relation_type array (all edges assigned this relation)
    relation_type_array = torch.tensor([relation_idx] * n_test_edges, dtype=torch.long)
    
    # Score
    scores = task.predict_edge_scores(
        data=napistu_data,
        edge_index=test_edges,
        relation_type=relation_type_array
    )
    
    # Store results
    for edge_i, score in enumerate(scores):
        results.append({
            'edge_idx': edge_i,
            'true_relation': relation_types[test_edges_true_relation_types[edge_i]],
            'scored_as_relation': relation_name,
            'score': score.item(),
        })
